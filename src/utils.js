...
export function percolatePath(movement, coords, direction, memo, state){
  if (!movement) return;
  const {y, x} = coords,
...
export function detectAdjacentWalls(state, piece, cell){
...
export function getNeighbors(board, cell, orientation){
    const { y, x } = cell,
    pathMap = [],
...
export function findPath(startX, startY, endX, endY, board, orientation, race) {
...
export function reorient(array, orientation){
...
export function findAdjacentCells(destination, pathMap){
...
export function satisfyConnection(piece, origin, target, board){
  if (piece === 'Knight'){
